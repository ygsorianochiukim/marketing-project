...
const gold = '#8a6a3b'
const goldLight = '#a8853f'

function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 36 }}>
      <p style={{ ...serif, fontStyle: 'italic', fontSize: 14, color: gold, letterSpacing: '0.2em', marginBottom: 6, textTransform: 'uppercase' }}>
        {title}
      </p>
      <hr style={{ border: 0, borderTop: '1px solid #c9b88f', marginBottom: 18 }} />
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
      <span style={{ ...serif, fontStyle: 'italic', fontSize: 14, color: '#5a5a5a', minWidth: 180 }}>{label} :</span>
      {children}
    </div>
  )
}
...
function TextInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <input
      ...
      style={{
        flex: 1, minWidth: 200, background: '#f3e8cf', border: '1px solid #c9b88f',
        borderRadius: 6, padding: '7px 12px', color: '#2d2a26',
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <input
        ...
        value={value}
        onChange={e => onChange(e.target.value)}
        style={{ width: 40, height: 36, border: '1px solid #c9b88f', borderRadius: 6, cursor: 'pointer', background: 'none', padding: 2 }}
      />
      <input
        value={value}
        onChange={e => onChange(e.target.value)}
        style={{
          width: 100, background: '#f3e8cf', border: '1px solid #c9b88f',
          borderRadius: 6, padding: '7px 10px', color: '#2d2a26',
...
export default function SettingsPage() {
  ...
  if (!settings) {
    return (
      <div style={{ minHeight: '100vh', background: '#e9ddc4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ ...serif, fontStyle: 'italic', color: '#5a5a5a', fontSize: 15 }}>Loading...</p>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#e9ddc4', color: '#2d2a26' }}>
      <div style={{ padding: '40px 24px', maxWidth: 760, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <a href="/dashboard" style={{ ...mono, fontSize: 11, color: '#5a5a5a', textDecoration: 'none', letterSpacing: '0.1em' }}>
            ← BACK
          </a>
          <p style={{ ...serif, fontSize: 14, color: gold, letterSpacing: '0.3em', marginTop: 24, marginBottom: 8, textTransform: 'uppercase' }}>
            Ad Generator
          </p>
          <h1 style={{ ...serif, fontSize: 56, fontWeight: 700, color: '#2d2a26', marginBottom: 8, letterSpacing: '-0.02em', lineHeight: 1 }}>
            Brand Settings
          </h1>
          <p style={{ ...serif, fontStyle: 'italic', fontSize: 15, color: '#5a5a5a' }}>
        ...
        <Section title="Brand Logo">
          <p style={{ ...serif, fontStyle: 'italic', fontSize: 14, color: '#5a5a5a', lineHeight: 1.6 }}>
            Preset logo used on every ad. PNG or SVG with transparent background works best.
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
            <div style={{
              width: 160, height: 80, background: '#ffffff', border: '1px solid #c9b88f',
              ...
              {(logoPreview || settings.logoUrl) ? (
                ...
                <span style={{ ...serif, fontStyle: 'italic', fontSize: 12, color: '#8a857a' }}>no logo set</span>
              )}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <label style={{
                display: 'inline-block', padding: '7px 18px',
                background: logoUploading ? 'transparent' : `linear-gradient(135deg, ${gold}, ${goldLight})`,
                border: `1px solid ${gold}`, borderRadius: 6,
                color: logoUploading ? gold : '#f3e8cf',
              ...
              {settings.logoUrl && (
                <button onClick={removeLogo} style={{
                  ...
                  border: '1px solid #a04848', borderRadius: 6, color: '#a04848',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
              <div style={{
                ...
                {(avatarPreview || settings.staffAvatarUrl) ? (
                  ...
                  <span style={{ ...serif, fontStyle: 'italic', fontSize: 11, color: '#8a857a', textAlign: 'center', padding: 4 }}>no photo</span>
                )}
              </div>
              <label style={{
                padding: '5px 14px',
                background: avatarUploading ? 'transparent' : `linear-gradient(135deg, ${gold}, ${goldLight})`,
                border: `1px solid ${gold}`, borderRadius: 6,
                color: avatarUploading ? gold : '#f3e8cf',
        ...
        <Section title="Extra Instructions for Claude">
          ...
          <textarea
            ...
            style={{
              width: '100%', background: '#f3e8cf', border: '1px solid #c9b88f',
              borderRadius: 6, padding: '10px 14px', color: '#2d2a26',
              fontFamily: 'DM Mono, monospace', fontSize: 12, outline: 'none',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 8 }}>
          <button
            ...
            style={{
              padding: '10px 28px',
              background: saving ? 'transparent' : `linear-gradient(135deg, ${gold}, ${goldLight})`,
              border: `1px solid ${gold}`,
              borderRadius: 6, color: saving ? gold : '#f3e8cf',
          ...
          {saved && (
            <span style={{ ...serif, fontStyle: 'italic', fontSize: 14, color: '#5a7a48' }}>
              ✓ Saved
            </span>
          )}
        </div>
